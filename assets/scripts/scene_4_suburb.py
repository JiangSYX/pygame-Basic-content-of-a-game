...
def scene_4_suburb(screen, scene_4_suburb_bool, open_homescene_bool, up_bool, down_bool, left_bool, right_bool, if_up, if_down, if_left, if_right, x_dvalue, y_dvalue, main_coordinate, player_coordinate, player_health, bullets_number, x, y, if_open_agnifying, if_shoot, read_main_coordinate, runs_number, draw_chapter_name, i, min_i, max_i, draw_menuscene, home_button_bool, backgame_button_bool, menuexit_button_bool, if_draw_bullets, bullets_coordinate, bullets_id, Bullets, Pills, chapter_id, Zombies, zombies_action, neural_network, iteration, data_id, training_state, training_data, enter_key, died_i, open_loadscene_bool, if_died, anti_aliasing, chapter_4_objects, draw_blood_strip):
    ...
    object_1_dictionaries = {'up_bool': up_bool, 'down_bool': down_bool, 'left_bool': left_bool, 'right_bool': right_bool, 'if_up': chapter_4_objects[ids]['if_up'], 'if_down': chapter_4_objects[ids]['if_down'], 'if_left': chapter_4_objects[ids]['if_left'], 'if_right': chapter_4_objects[ids]['if_right']}
    ...
    chapter_4_objects[ids]['if_up'], chapter_4_objects[ids]['if_down'], chapter_4_objects[ids]['if_left'], chapter_4_objects[ids]['if_right'] = object_1_dictionaries['if_up'], object_1_dictionaries['if_down'], object_1_dictionaries['if_left'], object_1_dictionaries['if_right']
    ...
    # 绘制死亡界面
    draw_menuscene, died_i, open_loadscene_bool, scene_4_suburb_bool, i, min_i, max_i, runs_number, chapter_id, main_coordinate, read_main_coordinate, player_health, bullets_number, Pills, Zombies, iteration, data_id, training_state, neural_network, enter_key, x_dvalue, y_dvalue, if_died, Bullets, zombies_action, if_up, if_down, if_left, if_right, up_bool, down_bool, left_bool, right_bool, if_open_agnifying, if_shoot = draw_died(
        screen, draw_menuscene, died_i, open_loadscene_bool, scene_4_suburb_bool, i, min_i, max_i, runs_number, chapter_id, main_coordinate, read_main_coordinate, player_health, bullets_number, Pills, Zombies, iteration, data_id, training_state, neural_network, enter_key, x_dvalue, y_dvalue, if_died, Bullets, zombies_action, if_up, if_down, if_left, if_right, up_bool, down_bool, left_bool, right_bool, if_open_agnifying, if_shoot)


    # 绘制菜单界面(玩家按下确定退出键后, 因为已经重新初始化了draw_menuscene, 此时应该继续绘制菜单)
    if draw_menuscene == True or (scene_4_suburb_bool == False and open_homescene_bool == True):
